pipeline {
    ...
    environment {
        DOCKERHUB_CREDENTIALS = credentials('dockerhub-cred')   // Jenkins DockerHub credential (username+token)
    ...
    stages {
        ...
        stage('Locate Project Root') {
            steps {
                script {
                    // Find the pom.xml location dynamically
    ...
    post {
        ...
        success {
            echo "✅ Pipeline completed successfully! App running at http://localhost:9090"
        }
    }
}
